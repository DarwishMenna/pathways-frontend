...
const Buttons = (props: Props): JSX.Element => {
    ...
    return (
        <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginBottom: 10 }}>
            {hasPreviousQuestion ? <PreviousButton {...props} /> : <EmptyComponent />}
            {hasNextQuestion ? <NextButton {...props} /> : <RecommendedButton {...props}/>}
        </View>
    );
};

const NextButton = (props: Props): JSX.Element => {
    const onPress = (): SetActiveQuestionAction => props.setActiveQuestion(props.activeQuestion.nextQuestionId);
    ...
    return (
        <Button style={applicationStyles.whiteTopazButton} onPress={onPress} iconRight>
            <Text style={textStyles.whiteTopazButton}><Trans>Next</Trans></Text>
            <Icon name={icon} style={{ color: colors.topaz }} />
        </Button>
    );
};

const PreviousButton = (props: Props): JSX.Element => {
    const onPress = (): SetActiveQuestionAction => props.setActiveQuestion(props.activeQuestion.previousQuestionId);
...
const RecommendedButton = (props: Props): JSX.Element => {
